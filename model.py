...
def nova_igra():
    tezavnost = random.choice([2, 3, 4])
    return Memory(tezavnost)
...
class Multiplayer:
    ...
    def __init__(self, stevilo_igralcev):
        stevilo_igralcev = int(input('Vnesi stevilo igralcev: '))
        self.stevilo_igralcev = stevilo_igralcev

        tezavnost = int(input('Vnesi 2, 3 ali 4: '))
        self.tezavnost = tezavnost
        
        self.igralci = [[0 for i in range(2)] for j in range(self.stevilo_igralcev)] #array z dvema stolpcema in x vrstic, rpi cemer je x stevilo igralcev
        
        for x in self.igralci:           
            x[1] = 0            #stevilo zmag igralca
    ...
    def izpis_tock(self):
        print('Igralec: | St. zmag:')
        for i in range(len(self.igralci)):
            print('      ', i, '|', self.igralci[i][1]) 
        return
    
    def zacni_igro(self):
        
        for x in self.igralci: 
            x[0] = Memory(self.tezavnost)       # za vsakega igralca ustvarimo nov objekr Memoryja
        
        
        while 1:
            i = 1              # stevec za igralce
            for x in self.igralci:
                print(i , '. igralec na potezi')
                
                x[0].ugibaj()
                
                if x[0].zmaga() == True:
                    x[1] += 1               # povecamo stevilo zmag treuntnemu igralcu
                    print('-------------------------------')
                    print('Konec igre!')
                    print('Zmagal je', i , '. igralec')
                    
                    self.izpis_tock()
                    
                    nova_runda = input('Zelis igrati se eno rundo? [y/n]')
                    if nova_runda == 'y' or nova_runda == 'yes':
                        self.zacni_igro()
                    else:
                        return
